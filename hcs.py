...
def selfcheck(
    ...
    return loop.run_until_complete(
        asyncSelfCheck(name, birth, area, schoolname, level, password, schoolnum, customloginname)
    )
...
async def asyncSelfCheck(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        login_result = await asyncUserLogin(
            name, birth, area, schoolname, level, password, session, schoolnum
        )
...
async def asyncUserLogin(
    ...
    try:
        schoolcode = school_infos["schulList"][int(schoolnum)]["orgCode"]
